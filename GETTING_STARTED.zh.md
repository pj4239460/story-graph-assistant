...
4. 配置导演设置：
   ...
   - **节奏偏好**：平静/平衡/紧张
   - **多样性惩罚**：避免重复的程度（0-100%）
   - **空闲后触发备选**（v0.7 新增）：连续N个空闲tick后触发备选storylet（默认3）
...
- **排序约束**（v0.7）：用 `requires_fired` 和 `forbids_fired` 实现任务链和互斥路径
- **备选机制**（v0.7）：当常规storylet无法触发时，自动触发氛围事件防止卡住

**v0.7 新功能详解：**

**排序约束** - 无需复杂前置条件即可实现叙事依赖
- `requires_fired: ["quest_start"]` - 只有在指定storylet触发后才能出现
- `forbids_fired: ["join_guild"]` - 只有在指定storylet未触发时才能出现
- 适用场景：任务链、互斥路径、故事分支

**备选Storylet** - 防止叙事卡顿
- `is_fallback: true` - 标记为备选事件
- 当连续N个tick没有常规storylet触发时自动启用
- 通常是氛围事件："天气变化"、"市场活动"、"时间流逝"
- 保持世界动态而不强制主线推进

### ✏️ Storylet 可视化编辑器（v0.8 新增！）

**Storylet 编辑器**提供了用户友好的可视化界面，无需手动编辑 JSON 文件即可创建和管理 storylets。

**访问编辑器：**
1. 打开你的项目
2. 导航到 **✏️ Storylets** 标签页
3. 你会看到两个子标签：
   - **📚 库** - 浏览和管理现有 storylets
   - **➕ 新建** - 用表单构建新 storylet

**使用库功能：**
- **搜索**：输入关键词按标题、ID或标签查找storylets
- **筛选**：快速按类型筛选
  - `全部` - 显示所有 storylets
  - `备选` - 仅显示备选 storylets
  - `一次性` - 仅显示一次性 storylets
  - `有冷却` - 有冷却时间的 storylets
  - `有顺序约束` - 有序列依赖的 storylets
- **卡片**：每个 storylet 显示为可展开的卡片，包含：
  - 属性徽章（备选、一次性、冷却、标签）
  - 指标（权重、强度变化、条件数、效果数）
  - 排序约束（需要已触发、禁止已触发）
  - 详细的条件和效果列表
- **操作**：
  - `✏️ 编辑` - 加载 storylet 到表单进行修改
  - `🗑️ 删除` - 删除 storylet（需要确认）
...
2. **属性**
   - `权重`（0.0-10.0）- 条件满足时的选择概率
   - `强度变化`（-1.0 到 +1.0）- 对叙事张力的影响
   - `冷却时间`（0-20 ticks）- 再次触发前的最短时间
   - `一次性`复选框 - 每次游戏只能触发一次
   - `备选`复选框 - 世界卡住时触发（没有常规 storylet 满足条件）
   - `标签` - 逗号分隔的标签，用于筛选

3. **排序约束（v0.7）**
   - `需要已触发` - 必须先触发的 storylet ID 列表（逗号分隔）
     - 示例：`intro_01, setup_02` 表示需要这两个先触发
   - `禁止已触发` - 如果已触发则阻止此 storylet 的 ID 列表（逗号分隔）
     - 示例：`bad_end_01` 表示如果那个结局已触发则不能触发此 storylet
   - 用于任务链和叙事依赖

4. **前置条件**（动态列表）
   - 点击 `➕ 添加条件` 添加每个条件
   - 每个条件指定：
     - `范围`：`world`（世界）| `character`（角色）| `relationship`（关系）
     - `目标`：实体ID（角色名、"world"等）
     - `路径`：嵌套属性路径（如 "vars.gold"、"attributes.strength"）
     - `操作符`：比较运算符（`==`、`!=`、`>`、`<`、`>=`、`<=`）
     - `值`：要比较的数值
   - 所有条件必须满足才能选择此 storylet
   - 点击 `🗑️ 移除` 删除条件
...
5. **效果**（动态列表）
   ...
   - 每个效果指定：
     ...
     - `操作`：变更操作
       ...
       - `multiply` - 乘以现有值
     - `值`：要应用的值
   - 所有效果在 storylet 触发时原子性应用
   - 点击 `🗑️ 移除` 删除效果

**表单操作：**
- `💾 保存故事块` - 创建/更新 storylet 并保存项目
- `🔄 重置表单` - 清除所有字段
- `📋 复制` - 保留表单数据但清除ID以便复制

**使用技巧：**
- 使用有意义的ID（如 `market_boom_01`、`quest_dragon_accept`）
- 按类型标记 storylets（`combat`、`economic`、`relationship`）以便筛选
- 仔细测试前置条件 - 如果条件不匹配，storylets 将不会出现
- 使用备选 storylets 防止"世界卡住"场景
- 排序约束对于任务链和故事序列非常强大
