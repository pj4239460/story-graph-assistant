...
4. 配置导演设置：
   ...
   - **节奏偏好**：平静/平衡/紧张
   - **多样性惩罚**：避免重复的程度（0-100%）
   - **空闲后触发备选**（v1.7.1 新增）：连续N个空闲tick后触发备选storylet（默认3）
...
- **排序约束**（v1.7.1）：用 `requires_fired` 和 `forbids_fired` 实现任务链和互斥路径
- **备选机制**（v1.7.1）：当常规storylet无法触发时，自动触发氛围事件防止卡住
